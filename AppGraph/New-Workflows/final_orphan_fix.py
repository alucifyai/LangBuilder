...
def final_orphan_fix():
    with open("/Users/dongmingjiang/GB/LangBuilder/AppGraph/New-Workflows/langbuilder_app_graph_enhanced_v4_with_85_rbac_flows.json") as f:
        data = json.load(f)
    ...
    for subsystem in data["subsystems"]:
        ...
        # Find orphaned nodes
        orphaned_nodes = [node_id for node_id in all_nodes_local if node_id not in edge_node_ids]
        ...
        if orphaned_nodes:
            ...
            for orphaned_node in orphaned_nodes:
                ...
                existing_nodes = [node_id for node_id in all_nodes_local if node_id != orphaned_node and node_id in edge_node_ids]
