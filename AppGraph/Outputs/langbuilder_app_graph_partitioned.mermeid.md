```mermaid
%% LangBuilder Partitioned Application Graph v2.2.0 (Subsystems view)
graph LR
    ...
    flow_dashboard -. |authenticates| .-> user_entity
    flow_editor -. |triggers_execution| .-> flow_execution_engine
    reactflow_canvas -. |uploads_assets| .-> file_management_system
    message_entity -. |real_time_updates| .-> websocket_sse_communication
    settings_page -. |manages_api_keys| .-> api_key_entity
    auth_guard -. |validates_sessions| .-> session_management_system
    template_gallery -. |loads_templates| .-> configuration_management
    graph_state_management -. |synchronizes_state| .-> real_time_event_system
    notification_system -. |logs_notifications| .-> logging_monitoring_system
    error_handling_system -. |security_validation| .-> security_access_control
    login_page -. |initializes_session| .-> application_lifecycle
    io_modal -. |manages_components| .-> component_management
    vertex_build_entity -. |stores_builds| .-> store_integration_system
    websocket_manager -. |tracks_events| .-> telemetry_analytics_system
    workspace_switcher -. |validates_workspace| .-> testing_quality_assurance
```
