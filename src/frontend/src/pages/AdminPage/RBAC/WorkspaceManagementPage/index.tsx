...
export default function WorkspaceManagementPage() {
  ...
  const [filterWorkspaceList, setFilterWorkspaceList] = useState(workspaceList.current);

  const { mutate: mutateGetWorkspaces, isPending, isIdle } = useGetWorkspaces({});
  ...
  function handleFilterWorkspaces(input: string) {
    ...
    if (input === "") {
      ...
      const filteredList = workspaceList.current.filter((workspace: Workspace) =>
  ...
  function handleEditWorkspace(workspaceId: string, workspace: Partial<Workspace>) {
  ...
  function handleToggleActive(checked: boolean, workspaceId: string, workspace: Workspace) {
  ...
  function handleNewWorkspace(workspaceData: { name: string; description?: string; is_active?: boolean }) {
            ...
            <>
              ...
              >
                <Table className="table-fixed outline-1">
                  ...
                  {!isPending && (
                    <TableBody>
                      {filterWorkspaceList.map((workspace: Workspace, index) => (
                        <TableRow key={workspace.id}>
                          <TableCell className="truncate py-2 font-medium">
                            <ShadTooltip content={workspace.name}>
                              <span className="cursor-default">{workspace.name}</span>
                            </ShadTooltip>
                          </TableCell>
                          <TableCell className="truncate py-2">
                            <ShadTooltip content={workspace.description || "No description"}>
                          ...
                          <TableCell className="relative left-1 truncate py-2 text-align-last-left">
                            ...
                            >
                              <ConfirmationModal.Content>
                                <span>
                                  Are you sure you want to {workspace.is_active ? "deactivate" : "activate"} this workspace?
                                </span>
                              </ConfirmationModal.Content>
                              <ConfirmationModal.Trigger>
                                <div className="flex w-fit">
                                  <CheckBoxDiv checked={workspace.is_active} />
                                </div>
                              </ConfirmationModal.Trigger>
                            </ConfirmationModal>
                          </TableCell>
                          ...
                          <TableCell className="flex w-[100px] py-2 text-right">
                            <div className="flex">
                              <WorkspaceManagementModal
                                ...
                                index={index}
                                onConfirm={(workspaceData) => {
                                  handleEditWorkspace(workspace.id, workspaceData);
                                }}
                              >
