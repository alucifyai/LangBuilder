...
function EnvironmentBuilder({ environment, projects, onSave, onCancel }: EnvironmentBuilderProps) {
  const [name, setName] = useState(environment?.name || "");
  const [description, setDescription] = useState(environment?.description || "");
  const [selectedProjectId, setSelectedProjectId] = useState(
    environment?.project_id || projects[0]?.id || ""
  );
  const [environmentType, setEnvironmentType] = useState<"development" | "staging" | "production">(
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <Select value={environmentType} onValueChange={(value) => setEnvironmentType(value as any)}>
              <SelectTrigger>
            ...
            <div>
              ...
              <Select value={resourceLimit} onValueChange={setResourceLimit}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="xlarge">X-Large (8 CPU, 16GB RAM)</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
      ...
      <div className="flex justify-end space-x-2 pt-4 border-t">
        ...
        <Button onClick={handleSave} disabled={!name.trim() || !selectedProjectId}>
...
function EnvironmentTable({ environments, projects, onEdit, onDelete, onDeploy }: EnvironmentTableProps) {
  ...
  const filteredEnvironments = useMemo(() => {
    return environments.filter((env) => {
      ...
      const matchesProject = filterProject === "all" || env.project_id === filterProject;
            ...
            <TableBody>
              {filteredEnvironments.map((env) => (
                <TableRow key={env.id}>
                  ...
                  <TableCell>
                    <Badge className={getEnvironmentColor(env.environment_type)}>
                      <IconComponent
                  ...
                  <TableCell>
                    <div className="flex space-x-1">
                      <Button
                        ...
                        title="Deploy"
                        disabled={env.environment_type === "production" && !confirm("Deploy to production?")}
                      >
...
export default function EnvironmentManagement() {
  ...
  const handleUpdateEnvironment = (envData: CreateEnvironmentRequest) => {
    ...
    setEnvironments(environments.map(e => e.id === editingEnvironment.id ? updatedEnvironment : e));
  ...
  const typeStats = environments.reduce((acc, env) => {
    ...
    <div className="h-full flex flex-col p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{typeStats.development || 0}</div>
            <p className="text-xs text-muted-foreground">
              Dev environments
            </p>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{typeStats.production || 0}</div>
            <p className="text-xs text-muted-foreground">
