...
class DatabaseService(Service):
    ...
    def _run_migrations(self, should_initialize_alembic, fix) -> None:
        ...
        with self.alembic_log_path.open("w", encoding="utf-8") as buffer:
            ...
            # Skip alembic check for SQLite databases as they store UUIDs as VARCHAR
            # which causes false positive schema mismatches
            if "sqlite" not in self.database_url.lower():
                ...
                logger.debug("Skipping alembic check for SQLite database")
    ...
    def _create_db_and_tables(connection) -> None:
        ...
        # Create only non-RBAC tables - let migrations handle RBAC tables
        # RBAC tables: role, permission, role_permission, user_role_assignment
        rbac_table_names = {'role', 'permission', 'role_permission', 'user_role_assignment'}

        for table in SQLModel.metadata.sorted_tables:
            # Skip RBAC tables - they will be created by migrations
            if table.name in rbac_table_names:
                logger.debug(f"Skipping RBAC table {table.name} - will be created by migrations")
                continue
