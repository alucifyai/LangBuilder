    ...
    def _run_migrations(self, should_initialize_alembic, fix) -> None:
        ...
        with self.alembic_log_path.open("w", encoding="utf-8") as buffer:
            ...
            try:
                ...
                logger.warning(f"Migration mismatch detected (continuing in development mode): {exc}")
                # TODO: Re-enable strict check in production
                # Temporarily allowing mismatch during RBAC development to handle disabled Group/SCIM tables
                if not fix:
                    pass  # Skip raising error during development
