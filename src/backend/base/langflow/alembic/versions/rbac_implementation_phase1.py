...
def upgrade() -> None:
    ...
    op.create_index('idx_flow_project', 'flow', ['project_id'])
    ...
    op.add_column('api_key', sa.Column('scope_type', sa.String(50), nullable=True))
    op.add_column('api_key', sa.Column('scope_id', postgresql.UUID(as_uuid=True), nullable=True))
    op.add_column('api_key', sa.Column('workspace_id', postgresql.UUID(as_uuid=True), nullable=True))
    op.create_index('idx_api_key_service_account', 'api_key', ['service_account_id'])
