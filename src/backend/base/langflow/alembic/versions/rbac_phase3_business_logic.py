...
def upgrade() -> None:
    ...
    op.create_table(
        'sso_configuration',
        sa.Column('id', sa.String(length=32), primary_key=True, default=lambda: str(uuid4()).replace('-', '')),
    ...
    # Create Service Account Token table for advanced token management
    op.create_table(
        'service_account_token',
        sa.Column('id', sa.String(length=32), primary_key=True, default=lambda: str(uuid4()).replace('-', '')),
        sa.Column('service_account_id', sa.String(length=32), sa.ForeignKey('service_account.id'), nullable=False, index=True),
        sa.Column('token_hash', sa.String(length=128), nullable=False, unique=True, index=True),
        sa.Column('name', sa.String(length=255), nullable=False),
        sa.Column('description', sa.Text(), nullable=True),
        sa.Column('scoped_permissions', JSON, nullable=True),  # List of specific permissions
        sa.Column('scope_type', sa.String(length=50), nullable=True),  # workspace, project, environment
        sa.Column('scope_id', sa.String(length=32), nullable=True, index=True),
        ...
        sa.Column('revoked_by_id', sa.String(length=32), sa.ForeignKey('user.id'), nullable=True),
    )
