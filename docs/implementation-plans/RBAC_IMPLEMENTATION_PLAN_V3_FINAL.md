# Granular Access Control & RBAC Implementation Plan (v3 - FINAL)
...
- ✅ Renamed all "Impact Subgraph (Design)" references to "Impact Subgraph (Design)"
...
**Impact Subgraph (Design) (v2 COMPLETE):**
```
Schema Nodes (NEW):
...
**Impact Subgraph (Design) (v2 UPDATED):**
```
Logic Nodes:
...
# Run automated scan
docker run -t owasp/zap2docker-stable zap-baseline.py   -t http://localhost:7860   -r zap_report.html
```
...
locust -f tests/performance/locustfile_rbac.py   --host http://localhost:7860   --users 10000   --spawn-rate 100   --run-time 10m   --html locust_report.html
```
...
async def test_required_indexes_exist(db_session):
    ...
    for table_name, columns in required_indexes:
        ...
        assert result.scalar() is not None,             f"Missing index on {table_name}({','.join(columns)})"
```
