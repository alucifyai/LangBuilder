...
| **2.6** | **Owner Role Enforcement** | **Scenario: Enforcing Owner Role**  **Given** the user has a Owner role for a scope entity  **When** the user tries to read, update, delete, execute, export, import, download the entity **Then** the user should be able to perform those actions |
| **2.7** | **Admin Role Enforcement** | **Scenario: Enforcing Admin Role**  **Given** the user has an Admin role **When** the user tries to read, update, delete, execute, export, import, download any entity **Then** the user should be able to perform those actions |
| **2.8** | **Editor Role Enforcement** | **Scenario: Enforcing Editor Role**  **Given** the user has an Editor role for a scope entity **When** the user tries to read, update, execute, export, import, download the entity **Then** the user should be able to perform those actions **And When** the user tries to delete an entity  **Then** the user should not be able to perform those actions |
| **2.9** | **Viewer Role Enforcement** | **Scenario: Enforcing Viewer Role**  **Given** the user has a Viewer role for a scope entity **When** the user tries to read, execute, export, download the entity **Then** the user should be able to perform the actions **And When** the user tries to update, delete, import the entity  **Then** the user should not be able to perform those actions |
| **2.10.** | **Default Project Owner Immutability Check** | **Scenario: Default Project Owner Immutability Check Given** a user has the **Owner** role assigned to their default/Starter Project  **When** an **Admin** attempts to modify, delete, or transfer this specific Owner role assignment  **Then** the attempt should be blocked at the application logic layer **And** the user should maintain the **Owner** role on their Starter Project |
| **2.11.** | **Global Project Creation & New Entity Owner Mutability** | **Scenario: Project Creation and New Entity Owner Assignment**  **Given** any authenticated user is logged in  **When** the user attempts to create a new Project or a Flow **Then** the user should be able to do so and the user automatically gets assigned the owner role for that new entity  |
| **2.12.** | **Define Project to Flow Role Extension Rule** | **Scenario: Establishing Project Role Inheritance Logic**  **Given** a user is trying to take an action on a Flow and does not have an permission for that action on the Flow **When** the user has the same permission for the containing Project **Then** the user should be able to perform that action |
|  |  |  |
